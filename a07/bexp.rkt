...
(define (bexp-template bexp)
  (cond [(number? bexp) ...]
        [(cons? bexp) (... (opnode-template (first bexp) (second bexp)))]))

;; opnode-template: (anyof 'AND 'OR 'XOR) (listof BExp) -> Any
(define (opnode-template op lobe)
  (cond [(empty? lobe) ...]
        [(symbol=? 'AND op) (... (bexp-template (first lobe))
                                 (opnode-template (rest lobe)))]
        [(symbol=? 'OR op) (... (bexp-template (first lobe))
                                 (opnode-template (rest lobe)))]
        [(symbol=? 'XOR op) (... (bexp-template (first lobe))
                                 (opnode-template (rest lobe)))]))
